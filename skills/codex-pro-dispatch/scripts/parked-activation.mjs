...
const stageFile=join(dirname(packetFile),".packet-stage-"+stage+"-"+targetSha256+".js");
try{const h=await fs.open(stageFile,"wx",384);try{await h.writeFile(bytes);await h.sync();}finally{await h.close();}}
catch(e){if(e.code!=="EEXIST")throw e;const existing=await privateBytes(stageFile,1048576);
if(!existing.equals(bytes))throw Error("Resident packet stage differs");}
const command="test \"$(/usr/bin/shasum -a 256 "+quote(stageFile)+" | /usr/bin/awk '{print $1}')\" = "+
quote(targetSha256)+" && /bin/cat "+quote(stageFile);
